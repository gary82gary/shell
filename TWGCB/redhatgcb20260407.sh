...
printf "${menu}**${number} 7)  ${menu}TWGCB-04-007 Apache HTTP Server 2.4 GCB v1.2           (update:2026/04/09)\n"
printf "${menu}**${number} 7A) ${menu}TWGCB-04-007 Apache HTTP Server 2.4 GCB v1.2 AutoSetup (update:2026/04/09)\n"
printf "${menu}**${number} 8)  ${menu}TWGCB-01-008 RedHat Enterprise Linux 8 V1.3            (update:2026/02/05)\n"
printf "${menu}**${number} 8A) ${menu}TWGCB-01-008 RedHat Enterprise Linux 8 V1.3  AutoSetup (buliding)\n"
printf "${menu}**${number} 12) ${menu}TWGCB-01-012 RedHat Enterprise Linux 9 V1.2            (update:2026/02/05)\n"
printf "${menu}**${number} 12A)${menu}TWGCB-01-012 RedHat Enterprise Linux 9 V1.2  AutoSetup (update:2026/02/05)\n"
...
# 清除日誌檔案
rm -f "$SCRIPT_DIR"/TWGCB-04-007-*.log 2>/dev/null



# --- TWGCB-04-007-0001：檢查 log_config_module 是否啟用 ---
# 說明：此模組負責處理 CustomLog 與 LogFormat，為 GCB 稽核紀錄之基礎。
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0001.log"

# 1. 檢查運行中模組 (Runtime Check)
# 使用 httpd -M 擷取目前載入的模組清單
LOADED_MODULES=$(httpd -M 2>/dev/null)
RUNNING_STATUS=$(echo "$LOADED_MODULES" | grep -q "log_config_module" && echo "ENABLED" || echo "DISABLED")

# 2. 檢查設定檔宣告 (Configuration Check)
# 確保 /etc/httpd/conf.modules.d/ 或是 httpd.conf 中沒有被註解掉
CONFIG_CHECK=$(grep -rE "^\s*LoadModule\s+log_config_module" /etc/httpd/conf* 2>/dev/null)

# 輸出結果與日誌
if [[ "$RUNNING_STATUS" == "ENABLED" ]] && [[ -n "$CONFIG_CHECK" ]]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0001 log_config_module is enabled."
else
    # 建立修復指引到 LOGFILE
    cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0001 log_config_module 修復指引
############################################################

檢測結果：系統未偵測到 log_config_module 載入。
此模組為 Apache 紀錄存取日誌所必須，若未啟用將無法符合 GCB 日誌規範。

修復步驟：
1. 檢查 /etc/httpd/conf.modules.d/00-base.conf
2. 確保以下行未被註解（移除開頭的 #）：
...
EOF
    ...
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0001 log_config_module is NOT enabled. (詳情請見: $LOGFILE)"
fi
 

# --- TWGCB-04-007-0002：停用 dav_module 與 dav_fs_module ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0002.log"
LOADED_MODULES=$(httpd -M 2>/dev/null)
# 檢查這兩個模組是否都沒有載入 (grep 找不到回傳 false 才是我們要的)
if ! echo "$LOADED_MODULES" | grep -qiE "dav_module|dav_fs_module"; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0002 disabled dav_module and dav_fs_module"
else
    cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0002 dav_module 修復指引
############################################################
檢測結果：偵測到啟用 WebDAV 相關模組。
WebDAV 容易產生安全性漏洞，若無特殊需求應予以關閉。

修復步驟：
1. 編輯 /etc/httpd/conf.modules.d/00-dav.conf
2. 將以下行開頭加上 # 註解掉：
   LoadModule dav_module modules/mod_dav.so
   LoadModule dav_fs_module modules/mod_dav_fs.so
   LoadModule dav_lock_module modules/mod_dav_lock.so
3. 執行 httpd -t 並重啟 systemctl restart httpd
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0002 need to be disabled dav_module. (見 $LOGFILE)"
fi
...
# --- TWGCB-04-007-0003：停用 mod_status ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0003.log"
if ! httpd -M 2>/dev/null | grep -q "status_module"; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0003 disabled mod_status"
else
    cat << EOF > "$LOGFILE"
修復指引：
請檢查 /etc/httpd/conf.modules.d/00-base.conf
註解掉：LoadModule status_module modules/mod_status.so
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0003 need to be disabled mod_status. (見 $LOGFILE)"
fi
...
# --- TWGCB-04-007-0004：停用 mod_autoindex ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0004.log"

if ! httpd -M 2>/dev/null | grep -q "autoindex_module"; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0004 disabled mod_autoindex"
else
    cat << EOF > "$LOGFILE"
修復指引：
請檢查 /etc/httpd/conf.modules.d/00-base.conf
註解掉：LoadModule autoindex_module modules/mod_autoindex.so
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0004 need to be disabled mod_autoindex. (見 $LOGFILE)"
fi

  #grep -r "mod_autoindex.so" /etc/httpd/
 
 
# --- TWGCB-04-007-0005：停用 proxy_module ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0005.log"
...
if ! httpd -M 2>/dev/null | grep -q "proxy_module"; then
    ...
    cat << EOF > "$LOGFILE"
修復指引：
請檢查 /etc/httpd/conf.modules.d/00-proxy.conf 及其它 proxy 相關設定檔
註解掉所有 LoadModule proxy_... 的行。
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0005 need to be disabled proxy_module. (見 $LOGFILE)"
fi
...
# --- TWGCB-04-007-0006：停用 userdir_module ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0006.log"
...
修復指引：
請檢查 /etc/httpd/conf.modules.d/00-base.conf
註解掉：LoadModule userdir_module modules/mod_userdir.so
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0006 need to be disabled userdir_module. (見 $LOGFILE)"
fi
...
# --- TWGCB-04-007-0007：停用 info_module ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0007.log"
...
修復指引：
請檢查 /etc/httpd/conf.modules.d/00-base.conf
註解掉：LoadModule info_module modules/mod_info.so
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0007 need to be disabled info_module. (見 $LOGFILE)"
fi
...
# --- TWGCB-04-007-0008：使用專屬帳號 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0008.log"
# 同時檢查設定檔中的 User/Group 設定與系統帳號是否存在
CONF_USER=$(grep -i "^User " /etc/httpd/conf/httpd.conf | awk '{print $2}')
if [[ "$CONF_USER" == "apache" ]] && id apache &>/dev/null; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0008 apache user is correctly configured."
else
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0008 User should be 'apache'. (當前: $CONF_USER)"
fi


# --- TWGCB-04-007-0009：鎖定 apache 帳號 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0009.log"
# 檢查帳號是否為鎖定狀態 (LK 或 L)
if passwd -S apache 2>/dev/null | grep -qE "LK|L"; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0009 apache account is locked."
else
    cat << EOF > "$LOGFILE"
修復指引：
GCB 要求執行服務的系統帳號必須被鎖定，禁止直接登入。
請執行：
passwd -l apache
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0009 apache account is NOT locked. (見 $LOGFILE)"
fi
#chsh -s /sbin/nologin apache

 
# --- TWGCB-04-007-0010：設定 apache 為 nologin ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0010.log"
if getent passwd apache | cut -d: -f7 | grep -q "nologin"; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0010 apache shell is set to nologin."
else
    cat << EOF > "$LOGFILE"
修復指引：
請將 apache 帳號的預設 Shell 改為不可登入：
usermod -s /sbin/nologin apache
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0010 apache shell is NOT nologin. (見 $LOGFILE)"
fi
 
 #passwd -l apache 
 
 
# --- TWGCB-04-007-0011 & 0012：ServerRoot 擁有者與群組 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0011_0012.log"

# 取得 ServerRoot 路徑，預設為 /etc/httpd
SERVER_ROOT=$(grep "^ServerRoot" /etc/httpd/conf/httpd.conf | awk '{print $2}' | tr -d '"')
[ -z "$SERVER_ROOT" ] && SERVER_ROOT="/etc/httpd"

OWNER_GROUP=$(stat -c "%U:%G" "$SERVER_ROOT")

if [[ "$OWNER_GROUP" == "root:root" ]]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0011/0012 ServerRoot owner is root:root."
else
    cat << EOF > "$LOGFILE"
修復指引：
ServerRoot 目錄及其內容應由 root 擁有，避免被 apache 帳號篡改。
請執行：
chown -R root:root $SERVER_ROOT
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0011/0012 ServerRoot owner is $OWNER_GROUP. (見 $LOGFILE)"
fi
 
 
# --- TWGCB-04-007-0013 & 0014：ServerRoot 權限 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0013_0014.log"

# 檢查是否除了 root 以外的帳號具有寫入權限 (排除符號連結)
W_PERM=$(find "$SERVER_ROOT" -perm /g+w,o+w ! -type l 2>/dev/null)

if [ -z "$W_PERM" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0013/0014 ServerRoot permissions are secure."
else
    cat << EOF > "$LOGFILE"
修復指引：
除 root 外，其餘帳號不應對 ServerRoot 有寫入權限。
發現以下檔案權限不符：
$W_PERM
...
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0013/0014 ServerRoot has insecure write permissions. (見 $LOGFILE)"
fi
 
 
# --- TWGCB-04-007-0015：DocumentRoot 權限 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0015.log"
# 取得 DocumentRoot，預設為 /var/www/html
DOC_ROOT=$(grep "^DocumentRoot" /etc/httpd/conf/httpd.conf | awk '{print $2}' | tr -d '"')
[ -z "$DOC_ROOT" ] && DOC_ROOT="/var/www/html"

# 檢查群組是否具有寫入權限 (GCB 要求不可有寫入權)
BAD_PERMS=$(find -L "$DOC_ROOT" -group apache -perm /g+w 2>/dev/null)

if [ -z "$BAD_PERMS" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0015 DocumentRoot permissions are secure."
else
    cat << EOF > "$LOGFILE"
修復指引：
DocumentRoot 下的檔案不應讓 apache 群組具有寫入權限，以防網頁被竄改。
請執行：
find -L $DOC_ROOT -group apache -perm /g+w -exec chmod g-w {} +
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0015 DocumentRoot has insecure group write permissions. (見 $LOGFILE)"
fi
...
# --- TWGCB-04-007-0016：停用 CoreDumpDirectory ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0016.log"

# 檢查是否有設定 CoreDumpDirectory (預設不應設定，以防洩漏記憶體資訊)
if ! grep -rEi "^\s*CoreDumpDirectory" /etc/httpd/conf* | grep -v "#" >/dev/null; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0016 CoreDumpDirectory is NOT set."
else
    cat << EOF > "$LOGFILE"
修復指引：
除非除錯需要，否則不應啟用 CoreDumpDirectory。
請註解掉設定檔中的 CoreDumpDirectory 行。
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0016 CoreDumpDirectory should be disabled. (見 $LOGFILE)"
fi
 
 #grep -r "CoreDumpDirectory" /etc/httpd/
 
 
# --- TWGCB-04-007-0017：Mutex 安全性 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0017.log"

# 1. 取得 Mutex 設定路徑 (排除註解)
# 格式通常為 Mutex file:/run/httpd default
MUTEX_VAL=$(grep -rEi "^\s*Mutex\s+file:" /etc/httpd/conf* | grep -v "#" | awk '{print $3}' | cut -d':' -f2 | head -1)

# 如果沒設定 Mutex，Apache 預設通常在 /run/httpd 或編譯路徑，視為 OK
if [ -z "$MUTEX_VAL" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0017 Mutex is using default secure path."
else
    # 取得目錄路徑 (去掉檔名前綴)
    MUTEX_DIR=$(dirname "$MUTEX_VAL")
    [ ! -d "$MUTEX_DIR" ] && MUTEX_DIR="$MUTEX_VAL" # 如果指向的是目錄
    ...
    if [[ "$MUTEX_DIR" == "$DOC_ROOT"* ]]; then
        ERR_MSG+="- 鎖定目錄不可位於 DocumentRoot ($DOC_ROOT) 下\n"
    fi

    # (2) 檢查擁有者與群組是否為 root:root
    OWNER_INFO=$(stat -c "%U:%G" "$MUTEX_DIR" 2>/dev/null)
    if [ "$OWNER_INFO" != "root:root" ]; then
        ERR_MSG+="- 目錄擁有者應為 root:root (目前: $OWNER_INFO)\n"
    fi

    # (3) 檢查寫入權限 (僅 root 可寫，即 755 或更嚴格)
    PERM=$(stat -c "%a" "$MUTEX_DIR" 2>/dev/null)
    if [ "$PERM" -gt 755 ]; then
        ERR_MSG+="- 目錄權限過大，僅 root 應具寫入權 (目前: $PERM)\n"
    fi

    # (4) 檢查是否為本機掛載 (排除 NFS/網路檔案系統)
    FSTYPE=$(df -T "$MUTEX_DIR" 2>/dev/null | tail -1 | awk '{print $2}')
    if [[ "$FSTYPE" == "nfs"* || "$FSTYPE" == "cifs"* ]]; then
        ERR_MSG+="- 目錄不可位於網路掛載點 (目前檔案系統: $FSTYPE)\n"
    fi

    # --- 判定結果 ---
    if [ -z "$ERR_MSG" ]; then
        echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0017 Mutex directory is secure."
    else
        cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0017 Mutex 安全性修復指引
############################################################
檢測路徑：$MUTEX_DIR
發現問題：
$ERR_MSG
...
建議修復步驟：
1. 修改 httpd.conf 中的 Mutex 指令，指向 /run/httpd/ (或預設位置)。
2. 確保目錄權限：
   chown root:root $MUTEX_DIR
   chmod 755 $MUTEX_DIR
EOF
        echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0017 Mutex settings insecure. (見 $LOGFILE)"
    fi
fi
...
# --- TWGCB-04-007-0018：保護 pid 檔案 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0018.log"

# 1. 取得 PidFile 路徑 (預設通常在 /run/httpd/httpd.pid) 
PID_FILE=$(grep -rEi "^\s*PidFile" /etc/httpd/conf* | grep -v "#" | awk '{print $2}' | tr -d '"' | head -1)
[ -z "$PID_FILE" ] && PID_FILE="/run/httpd/httpd.pid"
PID_DIR=$(dirname "$PID_FILE")

# 2. 取得 DocumentRoot 路徑 
DOC_ROOT=$(grep -i "^DocumentRoot" /etc/httpd/conf/httpd.conf | awk '{print $2}' | tr -d '"')
[ -z "$DOC_ROOT" ] && DOC_ROOT="/var/www/html"

# 3. 執行三項指標檢查 
ERR_MSG=""

# (1) 檢查是否在 DocumentRoot 中 
if [[ "$PID_DIR" == "$DOC_ROOT"* ]]; then
    ERR_MSG+="- pid 檔案目錄不可位於 DocumentRoot ($DOC_ROOT) 中\n"
fi

# (2) 檢查擁有者是否為 root:root 
OWNER_INFO=$(stat -c "%U:%G" "$PID_DIR" 2>/dev/null)
if [ "$OWNER_INFO" != "root:root" ]; then
    ERR_MSG+="- 目錄擁有者應為 root:root (目前: $OWNER_INFO)\n"
fi

# (3) 檢查「僅 root 具寫入權」 (檢查 Group 和 Others 是否有 w 權限) 
# 使用 stat %a 取得權限，並檢查後兩碼是否包含 2, 3, 6, 7 (即含有 w)
if stat -c "%A" "$PID_DIR" | grep -qE '^....w..w.'; then
    ERR_MSG+="- 除了 root 以外的帳號不應具有寫入權限\n"
fi

# --- 判定結果 ---
if [ -z "$ERR_MSG" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0018 PidFile directory is secure."
else
    cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0018 保護 pid 檔案修復指引
############################################################

什麼是「pid 檔案」？ 
Apache 啟動時會將主程式的 Process ID 記錄在此。如果目錄權限過大，
其他帳戶可以建立同名檔案導致伺服器無法啟動 。
...
2. 移除群組與其他人的寫入權限：
   chmod go-w $PID_DIR
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0018 PidFile directory insecure. (詳情見: $LOGFILE)"
fi
...
# --- TWGCB-04-007-0019：ScoreBoardFile 路徑 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0019.log"

SCORE_FILE=$(grep -rEi "^\s*ScoreBoardFile" /etc/httpd/conf* | grep -v "#" | awk '{print $2}' | tr -d '"')

if [ -z "$SCORE_FILE" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0019 ScoreBoardFile is not explicitly set (Default is secure)."
else
    DOC_ROOT=$(grep "^DocumentRoot" /etc/httpd/conf/httpd.conf | awk '{print $2}' | tr -d '"')
    if [[ "$SCORE_FILE" == *"$DOC_ROOT"* ]]; then
        cat << EOF > "$LOGFILE"
修復指引：
ScoreBoardFile 不可位於 DocumentRoot 下，以防內部資訊洩漏。
建議取消設定該指令，讓其維持在記憶體或預設安全路徑。
EOF
        echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0019 ScoreBoardFile path is insecure. (見 $LOGFILE)"
    else
        echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0019 ScoreBoardFile path is secure."
    fi
fi
 
 
# --- TWGCB-04-007-0020：<Directory /> 預設拒絕 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0020.log"

# 擷取主設定檔中對根目錄的定義區塊
DIR_ROOT_BLOCK=$(sed -n '/<Directory \/>/,/<\/Directory>/p' /etc/httpd/conf/httpd.conf)

if echo "$DIR_ROOT_BLOCK" | grep -q "Require all denied"; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0020 <Directory /> is set to 'Require all denied'."
else
    cat << EOF > "$LOGFILE"
修復指引：
請確保 /etc/httpd/conf/httpd.conf 中的 <Directory /> 區塊包含以下內容：
<Directory />
    AllowOverride none
    Require all denied
</Directory>
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0020 <Directory /> is NOT denying all by default. (見 $LOGFILE)"
fi


# --- TWGCB-04-007-0021：根目錄停用 AllowOverride ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0021.log"

# 擷取主設定檔中的 <Directory /> 區塊
DIR_ROOT_BLOCK=$(sed -n '/<Directory \/>/,/<\/Directory>/p' /etc/httpd/conf/httpd.conf)

# 檢查是否包含 AllowOverride None 且沒有 AllowOverrideList
if echo "$DIR_ROOT_BLOCK" | grep -iqE "^\s*AllowOverride\s+None" && ! echo "$DIR_ROOT_BLOCK" | grep -iq "AllowOverrideList"; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0021 <Directory /> AllowOverride is None."
else
    cat << EOF > "$LOGFILE"
修復指引：
根目錄應禁止任何覆蓋設定。請確保 /etc/httpd/conf/httpd.conf 的區塊如下：
<Directory />
    AllowOverride None
    Require all denied
</Directory>
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0021 <Directory /> AllowOverride settings insecure. (見 $LOGFILE)"
fi


# --- TWGCB-04-007-0022：全域檢查 AllowOverride ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0022.log"

# 搜尋所有設定檔，找出任何 AllowOverrideList 或 AllowOverride 不是 None 的地方 (排除註解)
BAD_AO=$(grep -rEi "^\s*(AllowOverrideList|AllowOverride\s+(?!None))" /etc/httpd/conf* | grep -v "#")

if [ -z "$BAD_AO" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0022 Global AllowOverride settings comply."
else
    cat << EOF > "$LOGFILE"
修復指引：
GCB 要求所有目錄預設皆不應允許覆蓋（None）。
發現以下不符設定：
$BAD_AO
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0022 Non-compliant AllowOverride found. (見 $LOGFILE)"
fi

 
# --- TWGCB-04-007-0023：根目錄 Options 設為 None ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0023.log"

DIR_ROOT_BLOCK=$(sed -n '/<Directory \/>/,/<\/Directory>/p' /etc/httpd/conf/httpd.conf)

if echo "$DIR_ROOT_BLOCK" | grep -iqE "^\s*Options\s+None"; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0023 <Directory /> Options is None."
else
    cat << EOF > "$LOGFILE"
修復指引：
根目錄不應啟用任何額外功能。請修改 /etc/httpd/conf/httpd.conf：
<Directory />
    Options None
    ...
</Directory>
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0023 <Directory /> Options not set to None. (見 $LOGFILE)"
fi


# --- TWGCB-04-007-0024：網頁目錄 Options 設為 None ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0024.log"

# 自動取得 DocumentRoot (對齊 GCB 要求的文件根目錄) 
DOC_ROOT=$(grep -i "^DocumentRoot" /etc/httpd/conf/httpd.conf | awk '{print $2}' | tr -d '"')
[ -z "$DOC_ROOT" ] && DOC_ROOT="/var/www/html"
...
for file in $FILES; do
    # 使用 grep -n 找出起始與結束行號，避免使用 awk 的 /.../ 匹配路徑斜線
    # 這裡支援有無引號的情況 
    START_LINE=$(grep -niE "<Directory\s+\"?$DOC_ROOT\"?>" "$file" | cut -d: -f1 | head -1)
    
    if [ -n "$START_LINE" ]; then
        # 從起始行開始往下抓到最近的一個 </Directory> 
        BLOCK=$(sed -n "${START_LINE},/<\/Directory>/p" "$file")
        
        # 檢查 Options 是否為 None (不分大小寫) 
        if ! echo "$BLOCK" | grep -iqE "^\s*Options\s+None"; then
            echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0024 $file：<Directory \"$DOC_ROOT\"> 缺少 Options None(見 $LOGFILE)"
            OK=0
            # 寫入詳細資訊到 Log 
            echo "檔案 $file 中區塊不符規範：" >> "$LOGFILE"
            echo "$BLOCK" >> "$LOGFILE"
        fi
    fi
done
...
if [ $OK -eq 1 ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0024 <Directory \"$DOC_ROOT\"> 已正確設為 Options None"
fi

# --- TWGCB-04-007-0025：禁止使用 Includes ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0025.log"

# 檢查所有開啟的 Options 裡面是否有 Includes 或其縮寫
BAD_OPTIONS=$(grep -rEi "^\s*Options\s+.*(Includes|INC)" /etc/httpd/conf* | grep -v "#")

if [ -z "$BAD_OPTIONS" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0025 Options do not contain Includes."
else
    cat << EOF > "$LOGFILE"
修復指引：
Server-Side Includes (SSI) 具有安全風險。請移除以下設定中的 Includes 字眼：
$BAD_OPTIONS
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0025 Insecure Options (Includes) found. (見 $LOGFILE)"
fi


# --- TWGCB-04-007-0026：移除預設網頁與手冊 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0026.log"
# 1. 取得文件根目錄
DOC_ROOT=$(grep -i "^DocumentRoot" /etc/httpd/conf/httpd.conf | awk '{print $2}' | tr -d '"')
[ -z "$DOC_ROOT" ] && DOC_ROOT="/var/www/html"

# 2. 檢查預設「歡迎」檔案 (排除掉您自己的 index.html)
# 官方預設測試頁通常名為 welcome.html 或在 welcome.conf 中定義
WELCOME_FILE=$(ls "$DOC_ROOT"/welcome.html 2>/dev/null)

# 3. 檢查是否安裝手冊套件
MANUAL_INSTALLED=$(rpm -q httpd-manual 2>/dev/null | grep -v "not installed")

# 4. 檢查 welcome.conf 是否存在 
WELCOME_CONF="/etc/httpd/conf.d/welcome.conf"

# 判定邏輯：只要 welcome.conf 消失且套件移除，通常就視為合規 
if [ -z "$WELCOME_FILE" ] && [ -z "$MANUAL_INSTALLED" ] && [ ! -f "$WELCOME_CONF" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0026 Default content and manual are removed."
else
    cat << EOF > "$LOGFILE"
修復指引：
GCB 要求移除所有預設內容以減少攻擊面。
請執行以下操作：
1. 移除測試網頁：rm -f $DOC_ROOT/welcome.html
2. 移除預設設定：rm -f /etc/httpd/conf.d/welcome.conf
3. 解除安裝手冊：yum remove httpd-manual
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0026 Default content found. (見 $LOGFILE)"
fi

# --- TWGCB-04-007-0027/0028：移除預設 CGI 腳本 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0027_0028.log"

# 尋找 cgi-bin 目錄
CGI_PATH=$(grep "ScriptAlias /cgi-bin/" /etc/httpd/conf/httpd.conf | awk '{print $3}' | tr -d '"')
[ -z "$CGI_PATH" ] && CGI_PATH="/var/www/cgi-bin/"

BAD_CGI=$(ls "$CGI_PATH"printenv "$CGI_PATH"test-cgi 2>/dev/null)

if [ -z "$BAD_CGI" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0027/0028 Default CGI scripts are removed."
else
    cat << EOF > "$LOGFILE"
修復指引：
預設的 CGI 腳本會洩漏系統環境變數。
...
$BAD_CGI
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0027/0028 Default CGI scripts found. (見 $LOGFILE)"
fi
 

# --- TWGCB-04-007-0029：目錄存取限制 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0029.log"

# 檢查 /var/www 目錄區塊是否包含 Require all denied
# 避免使用者存取到非公開的系統目錄
VAR_WWW_BLOCK=$(sed -n '/<Directory "\/var\/www">/,/<\/Directory>/p' /etc/httpd/conf/httpd.conf)

if echo "$VAR_WWW_BLOCK" | grep -q "Require all denied"; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0029 /var/www access is restricted."
else
    cat << EOF > "$LOGFILE"
修復指引：
請確保 /etc/httpd/conf/httpd.conf 中的 <Directory "/var/www"> 區塊設定為拒絕存取：
<Directory "/var/www">
    AllowOverride None
    Require all denied
</Directory>
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0029 /var/www access is NOT restricted. (見 $LOGFILE)"
fi

# --- TWGCB-04-007-0030：停用 TraceEnable ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0030.log"

if grep -riE "^\s*TraceEnable\s+Off" /etc/httpd/conf* | grep -v "#" >/dev/null; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0030 TraceEnable is Off."
else
    cat << EOF > "$LOGFILE"
修復指引：
HTTP TRACE 方法可用於跨站腳本攻擊 (XSS) 以獲取 Cookie。
請在 /etc/httpd/conf/httpd.conf 全域設定中加入：
TraceEnable Off
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0030 TraceEnable should be Off. (見 $LOGFILE)"
fi
 
 

# --- TWGCB-04-007-0031：強制 HTTP/1.1 與繼承設定 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0031.log"

# 1. 檢查 mod_rewrite 模組是否載入 (Runtime)
MOD_LOADED=$(httpd -M 2>/dev/null | grep -i "rewrite_module")

# 2. 檢查所有設定檔（含 Include 進來的）是否有完整的強制 HTTP/1.1 規則
#    三行規則必須同時存在於同一個檔案中才算合規
ALL_CONF_FILES=$(httpd -V 2>/dev/null \
    | grep "SERVER_CONFIG_FILE" \
    | sed 's/.*="\(.*\)"/\1/')

CONF_SEARCH_DIRS="/etc/httpd/conf /etc/httpd/conf.d"
MAIN_RULE=""

for file in $(find $CONF_SEARCH_DIRS -name "*.conf" 2>/dev/null); do
    # 去除註解行後，檢查三條規則是否同時存在於該檔案
    CLEANED=$(grep -v '^\s*#' "$file")
    HAS_COND=$(echo "$CLEANED"  | grep -Ei 'RewriteCond\s+%\{THE_REQUEST\}\s+!HTTP/1\\\.1\$')
    HAS_RULE=$(echo "$CLEANED"  | grep -Ei 'RewriteRule\s+\.\*\s+-\s+\[F\]')
    HAS_ENGINE=$(echo "$CLEANED" | grep -Ei 'RewriteEngine\s+On')
    if [ -n "$HAS_COND" ] && [ -n "$HAS_RULE" ] && [ -n "$HAS_ENGINE" ]; then
        MAIN_RULE="found"
        break
    fi
done

# 3. 逐一解析每個 VirtualHost 區塊，確認區塊內有 RewriteOptions Inherit
VHOST_FILES=$(grep -rl "<VirtualHost" $CONF_SEARCH_DIRS 2>/dev/null)
INHERIT_MISSING=""

for file in $VHOST_FILES; do
    # 用 awk 擷取每個 <VirtualHost>...</VirtualHost> 區塊，逐塊檢查
    BLOCK_COUNT=$(grep -ci "<VirtualHost" "$file")
    for i in $(seq 1 "$BLOCK_COUNT"); do
        BLOCK=$(awk "
            /<[Vv]irtual[Hh]ost/{count++; if(count==$i){in_block=1}}
            in_block{print}
            /<\/[Vv]irtual[Hh]ost>/{if(in_block){in_block=0}}
        " "$file")

        if ! echo "$BLOCK" | grep -Eqi 'RewriteOptions\s+Inherit'; then
            INHERIT_MISSING+="$file (VirtualHost block $i)"$'\n'
        fi
    done
done

# 4. 判定結果
if [ -n "$MOD_LOADED" ] && [ -n "$MAIN_RULE" ] && [ -z "$INHERIT_MISSING" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0031 HTTP/1.1 enforcement and VHost inherit are set."
else
    # 組裝各項失敗原因
    REASON=""
    [ -z "$MOD_LOADED" ] && REASON+="- mod_rewrite 模組未載入\n"
    [ -z "$MAIN_RULE"  ] && REASON+="- 未找到完整的 HTTP/1.1 強制規則（RewriteEngine On + RewriteCond + RewriteRule）\n"
    [ -n "$INHERIT_MISSING" ] && REASON+="- 以下 VirtualHost 區塊缺少 RewriteOptions Inherit：\n${INHERIT_MISSING}"

    cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0031 HTTP/1.1 強制規範修復指引
############################################################
檢測結果：部分設定不符合 HTTP/1.1 強制規範。

不合規原因：
$(echo -e "$REASON")

修復步驟：
1. 確保 /etc/httpd/conf.modules.d/00-base.conf 已啟用：
   LoadModule rewrite_module modules/mod_rewrite.so

2. 在 /etc/httpd/conf/httpd.conf 全域區段加入：
   RewriteEngine On
   RewriteCond %{THE_REQUEST} !HTTP/1\.1$
   RewriteRule .* - [F]

3. 在所有 VirtualHost 區塊內加入：
   RewriteEngine On
   RewriteOptions Inherit
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0031 HTTP/1.1 enforcement is incomplete. (見 $LOGFILE)"
fi
 
 
# --- TWGCB-04-007-0032：保護 .ht 檔案 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0032.log"

# 檢查是否有針對 .ht 檔案的 FilesMatch 拒絕設定
HT_PROTECT=$(sed -n '/<FilesMatch "\^\\.ht">/,/<\/FilesMatch>/p' /etc/httpd/conf/httpd.conf | grep "Require all denied")

if [ -n "$HT_PROTECT" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0032 .ht files are protected."
else
    cat << EOF > "$LOGFILE"
修復指引：
必須防止外部存取 .htaccess 或 .htpasswd。
請確保 /etc/httpd/conf/httpd.conf 包含：
<FilesMatch "^\.ht">
    Require all denied
</FilesMatch>
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0032 .ht files are NOT protected. (見 $LOGFILE)"
fi
...
# --- TWGCB-04-007-0034：禁止 IP 直接存取網站 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0034.log"

# 1. 檢查模組是否載入 (Runtime)
MOD_OK=$(httpd -M 2>/dev/null | grep -q "rewrite_module" && echo "YES" || echo "NO")

# 2. 檢查引擎與規則設定 (Configuration)
# 檢查是否有 RewriteEngine On 且有針對 HTTP_HOST 的過濾規則
ENGINE_OK=$(grep -rEi "^\s*RewriteEngine\s+On" /etc/httpd/conf* | grep -v "#" >/dev/null && echo "YES" || echo "NO")
RULE_OK=$(grep -rEi "RewriteCond\s+%{HTTP_HOST}" /etc/httpd/conf* | grep -v "#" >/dev/null && echo "YES" || echo "NO")

if [[ "$MOD_OK" == "YES" ]] && [[ "$ENGINE_OK" == "YES" ]] && [[ "$RULE_OK" == "YES" ]]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0034 IP access restriction is properly configured."
else
    cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0034 禁止 IP 存取修復指引
############################################################

檢測結果：未發現完整的 IP 存取限制規則。
這項設定是為了強迫使用者使用網域名稱 (awsyum.fia.gov.tw) 存取，防止掃描攻擊。

修復步驟：
1. 確保 /etc/httpd/conf.modules.d/00-base.conf 已載入 rewrite 模組。
2. 在 /etc/httpd/conf/httpd.conf 或 ssl.conf 的 VirtualHost 區塊內加入：

   RewriteEngine On
   # 如果 Host 不是 www.example.com 就拒絕 (F = Forbidden)
...
3. 執行 httpd -t 檢查後重啟。
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0034 Incomplete IP access restriction. (見 $LOGFILE)"
fi
 
 
# --- TWGCB-04-007-0035：Listen 指令指定特定 IP ---
LOGFILE="./TWGCB-04-007-0035.log"
CONF_PATH="/etc/httpd/conf*"

# 1. 抓取不合規的 Listen：
#    (A) 排除註解行
#    (B) 找出行首為 Listen，但後面「沒有冒號 : 」(代表只寫 Port) 
#    (C) 或者明確寫著監聽所有介面 0.0.0.0 或 [::]
BAD_LISTEN=$(grep -rEi "^\s*Listen\s+" $CONF_PATH | grep -v "#" | grep -vE "(:|\[|0\.0\.0\.0)")

# 2. 額外檢查是否含有 0.0.0.0 (監聽所有 IPv4)
ALL_INTERFACE=$(grep -rEi "^\s*Listen\s+0\.0\.0\.0" $CONF_PATH | grep -v "#")

if [ -z "$BAD_LISTEN" ] && [ -z "$ALL_INTERFACE" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0035 所有 Listen 指令均已指定特定 IP。"
else
    cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0035 Listen 指令不合規報告
############################################################
偵測到以下 Listen 指令未綁定具體 IP (監聽所有網卡)，增加暴露風險：

$BAD_LISTEN
$ALL_INTERFACE

修復建議：
請將 Listen 修改為「IP:Port」格式，例如：Listen 192.168.1.100:80
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0035 偵測到不安全監聽設定 (見 $LOGFILE)"
fi

 
# --- TWGCB-04-007-0036：防止點擊劫持 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0036.log"

# 檢查是否有設定 SAMEORIGIN 或 DENY
X_FRAME=$(grep -riE "^\s*Header\s+(always\s+)?(append|set)\s+X-Frame-Options\s+(SAMEORIGIN|DENY)" /etc/httpd/conf* | grep -v "#")
CSP_FRAME=$(grep -riE "^\s*Header\s+(always\s+)?(append|set)\s+Content-Security-Policy\s+.*frame-ancestors" /etc/httpd/conf* | grep -v "#")

if [ -n "$X_FRAME" ] && [ -n "$CSP_FRAME" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0036 Anti-Clickjacking headers are set."
else
    cat << EOF > "$LOGFILE"
修復指引：
請在 /etc/httpd/conf/httpd.conf 加入以下標頭以防止網站被惡意框架嵌入：
Header always append X-Frame-Options SAMEORIGIN
Header always append Content-Security-Policy "frame-ancestors 'self';"
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0036 Missing X-Frame-Options or CSP. (見 $LOGFILE)"
fi
 
 
# --- TWGCB-04-007-0037：LogLevel 設定 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0037.log"

# 檢查是否設定為 notice 或更嚴格 (如 warn, error)
LOG_LV=$(grep -rEi "^\s*LogLevel\s+notice" /etc/httpd/conf/httpd.conf | grep -v "#")

if [ -n "$LOG_LV" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0037 LogLevel is notice or higher."
else
    cat << EOF > "$LOGFILE"
修復指引：
請修改 /etc/httpd/conf/httpd.conf 中的 LogLevel 設定：
LogLevel notice core:info
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0037 LogLevel not set to notice. (見 $LOGFILE)"
fi
 
 
# --- TWGCB-04-007-0038：存取日誌格式 (使用 Combined 格式) ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0038.log"

# 檢查所有啟用的 CustomLog 是否都使用了 combined 綽號
# 排除註解行後，搜尋 CustomLog 且後面沒有 combined 的設定
BAD_LOG_CONFIG=$(grep -rEi "^\s*CustomLog" /etc/httpd/conf* | grep -v "#" | grep -vi "combined")

if [ -z "$BAD_LOG_CONFIG" ] && grep -rEi "^\s*CustomLog\s+.*combined" /etc/httpd/conf* | grep -v "#" >/dev/null; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0038 Access log format is combined."
else
    cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0038 存取日誌格式修復指引
############################################################

檢測結果：發現部分 CustomLog 指令未使用「combined」格式。
GCB 規範要求：必須使用組合（Combined）格式，以完整紀錄來源網頁 (Referer) 
及瀏覽器特徵 (User-Agent)，這對於追蹤惡意掃描與攻擊來源至關重要。

修復步驟：
1. 編輯主設定檔 /etc/httpd/conf/httpd.conf：
   搜尋「CustomLog」，將其修改為：
   CustomLog "logs/access_log" combined

2. 編輯 SSL 設定檔 /etc/httpd/conf.d/ssl.conf：
   在 <VirtualHost _default_:443> 區塊內，將 CustomLog 修改為：
   CustomLog "logs/ssl_access_log" combined

3. 注意事項：
   請確保檔案中已定義 LogFormat ... combined（通常為系統預設）。
   修改完畢後，請執行「httpd -t」確認語法正確，
   並執行「systemctl restart httpd」重啟服務生效。

偵測到不合規的設定行為：
$BAD_LOG_CONFIG
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0038 Access log not using combined format. (詳情見: $LOGFILE)"
fi


# --- TWGCB-04-007-0039：日誌輪替設定 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0039.log"

# 1. 檢查系統輪替設定檔是否存在
# 2. 檢查是否有設定 weekly (每週更換) 或 daily (每天更換)
# 3. 檢查 rotate 次數是否足夠 (GCB 要求至少保留 90 天，若每週換一次，rotate 至少要 13)
ROTATION_CHECK=$(grep -rE "rotate\s+[0-9]+" /etc/logrotate.d/httpd 2>/dev/null)
RETENTION_VAL=$(echo "$ROTATION_CHECK" | awk '{print $2}' | head -1)

if [ -f "/etc/logrotate.d/httpd" ] && [ -n "$RETENTION_VAL" ] && [ "$RETENTION_VAL" -ge 13 ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0039 Logrotate for httpd is correctly configured."
else
    cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0039 日誌輪替 (Logrotate) 修復指引
############################################################

檢測結果：日誌保留天數不足或未設定輪替規則。

什麼是「日誌輪替」？
簡單來說，就是「換新筆記本」：
1. 為了不讓日誌檔無限變大塞爆硬碟，系統會定期(如每週)把舊日誌改名備份，並開一個新檔案。
2. GCB 規範要求必須保留至少 90 天的紀錄。如果我們「每週」換一本，
   系統至少要保留「13 本」舊紀錄 (13週 x 7天 = 91天)。

修復步驟：
1. 編輯設定檔：vi /etc/logrotate.d/httpd
2. 確保內容包含以下關鍵參數：
   /var/log/httpd/*log {
       weekly          # 每週輪替一次
       rotate 13       # 保留 13 份舊檔案 (即保留 13 週)
       missingok       # 找不到檔案也沒關係，不要報錯
       notifempty      # 檔案是空的就不輪替
       sharedscripts
       postrotate      # 換完檔案後，通知 Apache 重新導向日誌輸出
...
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0039 Logrotate retention is insufficient. (詳情見: $LOGFILE)"
fi


# --- TWGCB-04-007-0040：載入 mod_ssl ---

if httpd -M 2>/dev/null | grep -q "ssl_module"; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0040 mod_ssl is loaded."
else
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0040 mod_ssl NOT loaded. (dnf install mod_ssl)"
fi


# --- TWGCB-04-007-0041：SSL 私鑰權限 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0041.log"

# 1. 從 ssl.conf 抓取私鑰路徑
KEY_PATH=$(grep -rEi "^\s*SSLCertificateKeyFile" /etc/httpd/conf.d/ssl.conf | grep -v "#" | awk '{print $2}' | tr -d '"' | head -1)

if [ -z "$KEY_PATH" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0041 No SSL Key found (SKIP)."
else
    # 檢查擁有者是否為 root 且權限是否為 400 (僅 root 可讀)
    OWNER=$(stat -c "%U:%G" "$KEY_PATH" 2>/dev/null)
    PERM=$(stat -c "%a" "$KEY_PATH" 2>/dev/null)

    if [ "$OWNER" == "root:root" ] && [ "$PERM" -le 600 ]; then
        echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0041 SSL Key permission is secure."
    else
        cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0041 SSL 私鑰權限修復指引
############################################################

什麼是「SSL 私鑰」？
它是你網站的身份證。如果被別人拿走，他就能偽裝成你的 YUM 主機。
GCB 要求：只有 root 帳號可以看這個檔案，連 apache 帳號都不行。

偵測路徑：$KEY_PATH
目前狀態：擁有者 $OWNER，權限 $PERM

修復步驟：
1. 修正擁有者：chown root:root $KEY_PATH
2. 縮減權限：chmod 400 $KEY_PATH
EOF
        echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0041 SSL Key permission insecure. (見 $LOGFILE)"
    fi
fi


# --- TWGCB-04-007-0042：禁用舊版 TLS 協定 (功能驗證版) ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0042.log"

# 1. 取得實體 IP 進行測試
VM_IP=$(ip -4 addr show | grep -oP '(?<=inet\s)\d+(\.\d+){3}' | grep -v "127.0.0.1" | head -1)

# 2. 執行期測試：嘗試用 TLS 1.1 連線
# 捕捉所有輸出以利後續判定
TEST_RESULT=$(openssl s_client -connect ${VM_IP}:443 -tls1_1 < /dev/null 2>&1)

# 3. 判定邏輯：只要連不上 TLS 1.1 (包含 handshake failure 或系統政策攔截) 就算 OK
if echo "$TEST_RESULT" | grep -Eq "handshake failure|no protocols available|Connect failed"; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0042 功能驗證通過：TLS 1.1 已被封鎖。"
else
    # 如果真的連上了 (出現 Protocol : TLSv1.1)，則是嚴重不合規
    cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0042 TLS 協定安全性修復指引
############################################################

檢測結果：實測發現伺服器「仍允許」TLS 1.1 連線！
這不符合 GCB 規範，舊版協定容易遭受攔截與攻擊。

[ 修正步驟 ]：
1. 檢查設定檔位置 (最常見錯誤)：
   請確保 SSLProtocol 指令是寫在 <VirtualHost _default_:443> 區塊「內部」。
   檔案路徑：/etc/httpd/conf.d/ssl.conf

2. 修改指令內容：
   將 SSLProtocol 修改為以下其中一種格式：
   (A) 官方建議白名單：SSLProtocol +TLSv1.2 +TLSv1.3
   (B) 強制排除黑名單：SSLProtocol all -SSLv3 -TLSv1 -TLSv1.1

3. 檢查系統加密政策 (RHEL 9 特有)：
   執行指令：update-crypto-policies --show
   如果顯示 DEFAULT，建議提升為 FUTURE 或手動在 Apache 排除。

4. 重啟服務生效：
   執行：systemctl restart httpd

[ 實測詳細輸出 ]：
$TEST_RESULT
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0042 實測失敗：TLS 1.1 仍可連線 (見 $LOGFILE)"
fi

# --- TWGCB-04-007-0043：加密套件強度 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0043.log"
CONF_PATH="/etc/httpd/conf.d/ssl.conf"

# 1. 檢查是否開啟優先順序 (忽略縮排，只要有這行且沒被註解)
CHECK_HONOR=$(grep -rEi "^\s*SSLHonorCipherOrder\s+On" $CONF_PATH | grep -v "#")

# 2. 檢查是否包含關鍵的禁用標記 (MD5, RC4, NULL, aNULL)
# 改用 -q (quiet mode) 只要找到任何一個合規的設定就算初步過關
CHECK_CIPHER=$(grep -rEi "^\s*SSLCipherSuite" $CONF_PATH | grep -v "#" | grep -Ei "!MD5" | grep -Ei "!RC4" | grep -Ei "!aNULL")

# 3. 判定邏輯
if [ -z "$CHECK_HONOR" ]; then
    REASON="[缺失] 找不到 SSLHonorCipherOrder On (未強制伺服器偏好)"
elif [ -z "$CHECK_CIPHER" ]; then
    REASON="[缺失] SSLCipherSuite 未包含必要的禁用標記 (!MD5, !RC4, !aNULL)"
else
    REASON=""
fi

if [ -z "$REASON" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0043 Cipher suites are secure."
else
    cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0043 加密套件不合規報告
############################################################
失敗原因：$REASON

目前設定：
$(grep -rEi "SSLHonorCipherOrder|SSLCipherSuite" $CONF_PATH | grep -v "#")

修復指引：
請確保 /etc/httpd/conf.d/ssl.conf 的 <VirtualHost> 區塊內包含：
    SSLHonorCipherOrder On
    SSLCipherSuite ECDHE:DHE:!NULL:!LOW:!SSLv2:!MD5:!RC4:!aNULL
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0043 Encryption settings failed. (見 $LOGFILE)"
fi

# --- TWGCB-04-007-0044：禁用不安全重協商 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0044.log"

if grep -rEi "^\s*SSLInsecureRenegotiation\s+Off" /etc/httpd/conf.d/ | grep -v "#" >/dev/null; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0044 SSL Insecure Renegotiation is Off."
else
    cat << EOF > "$LOGFILE"
修復指引：
請在 /etc/httpd/conf.d/ssl.conf 加入：
SSLInsecureRenegotiation Off
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0044 SSL Renegotiation is insecure. (見 $LOGFILE)"
fi



# --- TWGCB-04-007-0045：禁用 SSL 壓縮 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0045.log"
HTTPD_CONF="/etc/httpd/conf/httpd.conf"
SSL_CONF="/etc/httpd/conf.d/ssl.conf"
SEARCH_PATH="/etc/httpd/conf/ /etc/httpd/conf.d/"

# 1. 檢測：抓取最後一個生效值
# 使用 tail -n 1 確保拿到最後被載入(覆蓋)的那個
CURRENT_VAL=$(grep -rEi "^\s*SSLCompression\b" $SEARCH_PATH 2>/dev/null | grep -v "#" | awk '{print $NF}' | tail -n 1)

# 2. 判斷是否為 Off (不分大小寫)
if [[ "${CURRENT_VAL,,}" == "off" ]]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0045 SSLCompression 已經設定為 $CURRENT_VAL"
else
	cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0045 SSLCompression 禁用規範修復指引
############################################################
檢測結果：未發現明確的 SSLCompression Off 設定，或設定值不正確。
當前檢測值：${CURRENT_VAL:-"(未設定)"}

資安風險：
開啟 SSL 壓縮會使加密連線容易遭受 CRIME 攻擊，進而導致 Session Cookie 等加密數據洩漏。

修復建議（二選一）：
1. [推薦] 統一管理：
   在 /etc/httpd/conf/httpd.conf 的全域區段加入：
   SSLCompression Off

2. 模組化管理：
   在 /etc/httpd/conf.d/ssl.conf 加入：
   SSLCompression Off

注意：
- 修改後請執行 'httpd -t' 驗證語法。
- 確保所有虛擬主機 (VirtualHost) 未重複定義或將其設定為 On。
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0045 SSL Compression 不合規 (見 $LOGFILE)"
fi



# --- TWGCB-04-007-0046：啟用 OCSP Stapling ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0046.log"
SSL_CONF="/etc/httpd/conf.d/ssl.conf"
SEARCH_PATH="/etc/httpd/conf/ /etc/httpd/conf.d/"

# 1. 檢測全域快取設定 (SSLStaplingCache)
# 必須存在於全域(不在 VHost 內)
HAS_CACHE=$(grep -rEi "^\s*SSLStaplingCache\b" $SEARCH_PATH 2>/dev/null | grep -v "#")

# 2. 檢測每個 VirtualHost 內部是否啟用 (SSLUseStapling)
# 使用 awk 檢查所有 VHost 區塊內容
VHOST_MISSING=$(awk '
    BEGIN { v_count=0; err=0 }
    /<[Vv]irtual[Hh]ost/ { in_v=1; has_s=0; v_count++ }
    in_v { if ($0 ~ /SSLUseStapling[[:space:]]+On/I) has_s=1 }
    /<\/[Vv]irtual[Hh]ost>/ { if (in_v && !has_s) err++; in_v=0 }
    END { if (v_count > 0 && err > 0) print "MISSING"; else if (v_count == 0) print "NO_VHOST"; else print "OK" }
' $SSL_CONF 2>/dev/null)

# 3. 判定結果
if [ -n "$HAS_CACHE" ] && [ "$VHOST_MISSING" == "OK" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0046 OCSP Stapling 配置正確。"
else
    cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0046 OCSP Stapling 修復指引
############################################################
檢測狀態：
- 全域快取 (SSLStaplingCache)：$( [ -n "$HAS_CACHE" ] && echo "已設定" || echo "未設定" )
- VHost 啟用 (SSLUseStapling)：$( [ "$VHOST_MISSING" == "OK" ] && echo "已合規" || echo "不合規或未發現" )

修復關鍵：
1. 【必須在區塊外】
   SSLStaplingCache 必須定義在 httpd.conf 或 ssl.conf 的全域層級。
   範例：SSLStaplingCache "shmcb:/run/httpd/stapling_cache(128000)"

2. 【必須在區塊內】
   每個 <VirtualHost> 區塊內部都必須明確加上：
   SSLUseStapling On

原因：
啟用 OCSP Stapling (裝訂) 可由伺服器預先向 CA 查詢憑證狀態並快取，
避免用戶端自行查詢造成的延遲與隱私問題，是現代 HTTPS 的標準加固項。
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0046 OCSP Stapling NOT configured. (見 $LOGFILE)"
fi



# --- TWGCB-04-007-0047：啟用 HSTS ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0047.log"

# 檢查 Header 裡是否有 Strict-Transport-Security 且 max-age 夠長
HSTS=$(grep -rEi "Strict-Transport-Security" /etc/httpd/conf.d/ | grep -v "#" | grep "max-age")

if [ -n "$HSTS" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0047 HSTS is enabled."
else
    cat << EOF > "$LOGFILE"
修復指引：
HSTS 可防止攻擊者將您的 HTTPS 網頁強制降級為不安全的 HTTP。
請在 ssl.conf 的 <VirtualHost _default_:443> 中加入：
Header always set Strict-Transport-Security "max-age=15768000"
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0047 HSTS NOT enabled. (見 $LOGFILE)"
fi


# --- TWGCB-04-007-0048：隱藏伺服器詳細版本 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0048.log"

if grep -rEi "^\s*ServerTokens\s+Prod" /etc/httpd/conf* | grep -v "#" >/dev/null; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0048 ServerTokens is Prod."
else
    cat << EOF > "$LOGFILE"
修復指引：
不應洩漏 Apache 的詳細版本（如 2.4.53），以免駭客針對特定漏洞攻擊。
請在 httpd.conf 加入：
ServerTokens Prod
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0048 ServerTokens is leaking info. (見 $LOGFILE)"
fi



# --- TWGCB-04-007-0049：關閉伺服器簽名 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0049.log"

if grep -rEi "^\s*ServerSignature\s+Off" /etc/httpd/conf* | grep -v "#" >/dev/null; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0049 ServerSignature is Off."
else
    cat << EOF > "$LOGFILE"
修復指引：
請在 httpd.conf 加入：
ServerSignature Off
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0049 ServerSignature is enabled. (見 $LOGFILE)"
fi




# --- TWGCB-04-007-0050：移除 Icons 別名 (精準版) ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0050.log"

# 1. 精準搜尋：只找 *.conf 檔案，且排除註解
# 使用 find 配合 grep 或是直接在 grep 限制副檔名
ICONS=$(grep -rEi "^\s*Alias\s+/icons/" /etc/httpd/conf/ /etc/httpd/conf.d/*.conf 2>/dev/null | grep -v "#")

if [ -z "$ICONS" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0050 Default Icons alias is disabled."
else
    cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0050 移除 Icons 別名修復指引
############################################################
檢測結果：發現未註解的 /icons/ 別名。
這會暴露伺服器的預設圖示路徑，增加資訊洩漏風險。

不合規項目：
$ICONS

修復步驟：
請在 /etc/httpd/conf.d/autoindex.conf 中註解掉：
#Alias /icons/ "/usr/share/httpd/icons/"
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0042 Icons alias is enabled. (見 $LOGFILE)"
fi
...
# --- TWGCB-04-007-0051：禁用 FileETag ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0051.log"

# 檢查是否設定為 None
ETAG_CONFIG=$(grep -rEi "^\s*FileETag\s+None" /etc/httpd/conf* | grep -v "#")

if [ -n "$ETAG_CONFIG" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0051 FileETag is set to None."
else
    cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0051 FileETag 修復指引
############################################################

什麼是「ETag」？
它是伺服器給檔案的一個「指紋」。
預設的指紋包含檔案在硬碟上的 Inode 編號，這會洩漏系統底層資訊。
GCB 要求關閉它以減少資訊洩漏。
...
2. 加入或修改以下指令：
   FileETag None
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0051 FileETag should be None. (見 $LOGFILE)"
fi
...
# --- TWGCB-04-007-0052：連線逾時 Timeout ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0052.log"

# 取得 Timeout 數值 (GCB 建議 60 秒以下)
TIMEOUT_VAL=$(grep -rEi "^\s*Timeout\s+" /etc/httpd/conf/httpd.conf | grep -v "#" | awk '{print $2}' | head -1)

if [ -n "$TIMEOUT_VAL" ] && [ "$TIMEOUT_VAL" -le 60 ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0052 Timeout is secure ($TIMEOUT_VAL)."
else
    cat << EOF > "$LOGFILE"
修復指引：
Timeout 設得太長會佔用系統資源。GCB 建議設為 60 秒或更短。
請在 httpd.conf 修改：
Timeout 60
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0052 Timeout too long or not set. (見 $LOGFILE)"
fi


# --- TWGCB-04-007-0053：開啟 KeepAlive ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0053.log"

if grep -rEi "^\s*KeepAlive\s+On" /etc/httpd/conf/httpd.conf | grep -v "#" >/dev/null; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0053 KeepAlive is On."
else
    cat << EOF > "$LOGFILE"
修復指引：
為了傳輸效率與安全性，應開啟 KeepAlive。
請在 /etc/httpd/conf/httpd.conf 設定：
KeepAlive On
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0053 KeepAlive should be On. (見 $LOGFILE)"
fi
...
# --- TWGCB-04-007-0054：MaxKeepAliveRequests (強效版) ---
HTTPD_CONF="/etc/httpd/conf/httpd.conf"
CONF_SEARCH_PATH="/etc/httpd/conf/ /etc/httpd/conf.d/"
TARGET_VAL=100

# 1. 取得最後生效的值 (跨檔案掃描)
CUR_VAL=$(grep -rEi "^\s*MaxKeepAliveRequests\b" $CONF_SEARCH_PATH 2>/dev/null | grep -v "#" | awk '{print $NF}' | tail -n 1)

# 2. 判斷邏輯 (規範要求 100 以上)
if [ -n "$CUR_VAL" ] && [ "$CUR_VAL" -ge "$TARGET_VAL" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0054 MaxKeepAliveRequests 已合規 ($CUR_VAL)"
else
    # 3. 執行修復：清理並唯一化
    find /etc/httpd/conf/ /etc/httpd/conf.d/ -name "*.conf" -exec sed -i '/^\s*MaxKeepAliveRequests\b/Id' {} +
    echo -e "\n# TWGCB-04-007-0054\nMaxKeepAliveRequests $TARGET_VAL" >> "$HTTPD_CONF"
    
    restorecon -v "$HTTPD_CONF"
    echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0054 已統一設置 MaxKeepAliveRequests=$TARGET_VAL"
fi

# --- TWGCB-04-007-0055：KeepAliveTimeout (強效版) ---
HTTPD_CONF="/etc/httpd/conf/httpd.conf"
CONF_SEARCH_PATH="/etc/httpd/conf/ /etc/httpd/conf.d/"
TARGET_TIMEOUT=15

# 1. 取得最後生效的值
CUR_VAL=$(grep -rEi "^\s*KeepAliveTimeout\b" $CONF_SEARCH_PATH 2>/dev/null | grep -v "#" | awk '{print $NF}' | tail -n 1)

# 2. 判斷邏輯 (規範要求 15 秒以下且大於 0)
if [ -n "$CUR_VAL" ] && [ "$CUR_VAL" -le "$TARGET_TIMEOUT" ] && [ "$CUR_VAL" -gt 0 ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0055 KeepAliveTimeout 已合規 ($CUR_VAL)"
else
    # 3. 執行修復：清理並唯一化
    find /etc/httpd/conf/ /etc/httpd/conf.d/ -name "*.conf" -exec sed -i '/^\s*KeepAliveTimeout\b/Id' {} +
    echo -e "\n# TWGCB-04-007-0055\nKeepAliveTimeout $TARGET_TIMEOUT" >> "$HTTPD_CONF"
    
    restorecon -v "$HTTPD_CONF"
    echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0055 已統一設置 KeepAliveTimeout=$TARGET_TIMEOUT"
fi
...
# --- TWGCB-04-007-0056/0057：請求讀取逾時設定 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0056_0057.log"

# 1. 檢查模組是否載入
MOD_OK=$(httpd -M 2>/dev/null | grep -q "reqtimeout_module" && echo "YES" || echo "NO")

# 2. 檢查設定值 (Header 至少 20-40秒, Body 至少 20秒)
TIMEOUT_CFG=$(grep -rEi "^\s*RequestReadTimeout" /etc/httpd/conf* | grep -v "#")

if [[ "$MOD_OK" == "YES" ]] && echo "$TIMEOUT_CFG" | grep -q "header=" && echo "$TIMEOUT_CFG" | grep -q "body="; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0056/0057 RequestReadTimeout is configured."
else
    cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0056/0057 RequestReadTimeout 修復指引
############################################################

什麼是「慢速連線攻擊 (Slowloris)」？
想像有人打電話到你的客服中心，接通後卻每 10 秒才講一個字。
他只要打幾十通電話，你的客服線路就會被佔滿。
這個指令就是規定：幾秒內沒講完重點（Header/Body），我們就掛電話。

修復步驟：
1. 確保 /etc/httpd/conf.modules.d/00-base.conf 已啟用：
   LoadModule reqtimeout_module modules/mod_reqtimeout.so

2. 在 /etc/httpd/conf/httpd.conf 加入以下建議值：
   RequestReadTimeout header=20-40,MinRate=500 body=20,MinRate=500
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0056/0057 ReqTimeout not set properly. (見 $LOGFILE)"
fi
 
 

# --- TWGCB-04-007-0058：限制請求行長度 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0058.log"

# GCB 建議設定，若沒設則 Apache 預設通常過大
L_LINE=$(grep -rEi "^\s*LimitRequestLine" /etc/httpd/conf* | grep -v "#" | awk '{print $2}' | head -1)

if [ -n "$L_LINE" ] && [ "$L_LINE" -le 8190 ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0058 LimitRequestLine is secure ($L_LINE)."
else
    cat << EOF > "$LOGFILE"
修復指引：
請在 httpd.conf 加入限制（建議 8190 或更小）：
LimitRequestLine 8190
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0058 LimitRequestLine too large or not set. (見 $LOGFILE)"
fi

 
 
# --- TWGCB-04-007-0059/0060：限制請求標頭 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0059_0060.log"

L_FIELDS=$(grep -rEi "^\s*LimitRequestFields" /etc/httpd/conf* | grep -v "#" | awk '{print $2}' | head -1)
L_FSIZE=$(grep -rEi "^\s*LimitRequestFieldSize" /etc/httpd/conf* | grep -v "#" | awk '{print $2}' | head -1)

if [ -n "$L_FIELDS" ] && [ "$L_FIELDS" -le 100 ] && [ -n "$L_FSIZE" ] && [ "$L_FSIZE" -le 8190 ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0059/0060 Request Headers limits are secure."
else
    cat << EOF > "$LOGFILE"
修復指引：
請在 httpd.conf 加入以下限制：
LimitRequestFields 100
LimitRequestFieldSize 8190
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0059/0060 Request Headers limits insecure. (見 $LOGFILE)"
fi
  
 
# --- TWGCB-04-007-0061：限制請求主體大小 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0061.log"

# GCB 建議至少在根目錄或特定目錄設定
L_BODY=$(grep -rEi "^\s*LimitRequestBody" /etc/httpd/conf* | grep -v "#" | awk '{print $2}' | head -1)

if [ -n "$L_BODY" ] && [ "$L_BODY" -le 1073741824 ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0061 LimitRequestBody is configured ($L_BODY bytes)."
else
    cat << EOF > "$LOGFILE"
修復指引：
請在 httpd.conf 的 <Directory /> 或 <Directory "/var/www/html"> 加入限制。
如果你不需要接收上傳檔案，建議設為 1GB (1073741824) 或更小：
LimitRequestBody 1073741824
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0061 LimitRequestBody not set. (見 $LOGFILE)"
fi


# --- TWGCB-04-007-0062：SELinux 狀態 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0062.log"

# 檢查目前運行狀態與設定檔狀態
CURRENT_MODE=$(getenforce 2>/dev/null)
CONFIG_MODE=$(grep -i "^SELINUX=enforcing" /etc/selinux/config 2>/dev/null)

if [[ "$CURRENT_MODE" == "Enforcing" ]] && [[ -n "$CONFIG_MODE" ]]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0062 SELinux is Enforcing."
else
    cat << EOF > "$LOGFILE"
############################################################
# TWGCB-04-007-0062 SELinux 修復指引
############################################################

什麼是「SELinux」？
它是 Linux 的保全系統。就算駭客破解了 Apache，SELinux 也會把他關在網頁目錄裡，
讓他無法去讀取你的密碼檔或修改系統設定。

修復步驟：
1. 編輯 /etc/selinux/config
2. 修改為：SELINUX=enforcing
3. 執行指令立即生效（暫時性）：setenforce 1
4. 注意：若原本是 Disabled，改為 Enforcing 後必須重啟系統。
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0062 SELinux is NOT Enforcing. (見 $LOGFILE)"
fi

 
# --- TWGCB-04-007-0063：Apache 執行檔 SELinux 標籤 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0063.log"

# 使用 stat 取得正確的類型欄位
HTTPD_TYPE=$(stat -c %C /usr/sbin/httpd 2>/dev/null | cut -d':' -f3)

if [[ "$HTTPD_TYPE" == "httpd_exec_t" ]]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0063 httpd binary context is correct."
else
    cat << EOF > "$LOGFILE"
修復指引：
Apache 執行檔的 SELinux 標籤錯誤，這會導致安全防禦失效。
請執行修復指令：
restorecon -v /usr/sbin/httpd
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0063 httpd context incorrect ($HTTPD_TYPE). (見 $LOGFILE)"
fi
 
 
 
# --- TWGCB-04-007-0064：禁止 httpd 寬容模式 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0064.log"

# 檢查是否有針對 httpd 的 permissive 設定
PERMISSIVE_CHECK=$(semanage permissive -l 2>/dev/null | grep "httpd_t")

if [ -z "$PERMISSIVE_CHECK" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0064 httpd is NOT in permissive mode."
else
    cat << EOF > "$LOGFILE"
修復指引：
偵測到 httpd 運行在 SELinux 寬容模式，這會讓防禦形同虛設。
請執行移除指令：
semanage permissive -d httpd_t
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0064 httpd is in permissive mode. (見 $LOGFILE)"
fi
 
 
 
# --- TWGCB-04-007-0065：禁用不安全認證模組 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0065.log"

LOADED_AUTH=$(httpd -M 2>/dev/null | grep -Ei "auth_basic_module|auth_digest_module")

if [ -z "$LOADED_AUTH" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0065 Insecure auth modules are disabled."
else
    cat << EOF > "$LOGFILE"
修復指引：
偵測到載入了 Basic 或 Digest 認證模組。
請在 /etc/httpd/conf.modules.d/00-base.conf 中註解掉：
#LoadModule auth_basic_module modules/mod_auth_basic.so
#LoadModule auth_digest_module modules/mod_auth_digest.so
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0065 Insecure auth modules loaded. (見 $LOGFILE)"
fi
 
 
# --- TWGCB-04-007-0066：強制 HTTPS 重定向 ---
LOGFILE="$SCRIPT_DIR/TWGCB-04-007-0066.log"

# 檢查是否有 Redirect 或 Rewrite 規則將 80 導向 443
REDIRECT_CHECK=$(grep -rEi "Redirect\s+permanent\s+/\s+https://" /etc/httpd/conf* | grep -v "#")

if [ -n "$REDIRECT_CHECK" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0066 HTTPS redirection is configured."
else
    cat << EOF > "$LOGFILE"
修復指引：
應強制所有流量走加密連線。
請在 /etc/httpd/conf/httpd.conf 的 <VirtualHost *:80> 區塊加入：
Redirect permanent / https://awsyum.fia.gov.tw/
EOF
    echo -e " [\e[1;31mKO\e[0m] TWGCB-04-007-0066 Missing HTTPS redirection. (見 $LOGFILE)"
fi
...
find "$SCRIPT_DIR" -maxdepth 1 -type f -name "TWGCB-04-007*.log" -size 0 -delete 2>/dev/null
...
# --- apache自動修復子選單函式 ---
show_apache_fix_menu() {
    while true; do
        echo -e "${YELLOW}"
        echo "============================================================"
        echo "      7A) Apache HTTP Server 2.4 GCB v1.2 自動修復子選單"
        echo "============================================================"
        echo -e "${NC}"
        echo -e "${menu}  1)${normal} 自動修復 + 執行預防性備份 (最安全)"
        echo -e "${menu}  2)${normal} 僅執行自動修復 (不備份)"
        echo -e "${menu}  3)${normal} 執行/etc/httpd/備份"
		echo -e "${menu}  4)${normal} 還原/etc/httpd/"
		echo -e "${menu}  5)${normal} 測試目前apache服務是否正常"
        echo "------------------------------------------------------------"
        echo -e "${menu}  EXIT)${normal} 返回主選單"
        echo "------------------------------------------------------------"
        echo -e "${YELLOW}修復內容包含：${NC}"
        echo -e "權限加固、加密協議(TLS 1.2+)、OCSP Stapling、HSTS、"
        echo -e "禁止壓縮、日誌格式、限制連線數等 (0002-0065)"
        
        read -p "請選擇執行項目: " apache_choice

        case $apache_choice in
            1)
                clear
				echo "執行前測試服務是否正常..."
				verify_and_restart_httpd
                echo "步驟 1/2：執行預防性備份..."
                quick_backup_httpd
                echo "----------------------------------------"
                echo "步驟 2/2：開始執行 Apache GCB 自動修復..."
                run_apache_autosetup
				echo -e " [\e[1;32m修復並備份完成！${NC}"
                read -p "按 Enter 返回子選單..."
                ;;
            2)
                clear
				echo "執行前測試服務是否正常..."
				verify_and_restart_httpd
                echo "警告：將不備份直接執行修復..."
                echo "開始執行 Apache GCB 自動修復..."
                run_apache_autosetup
				echo -e " [\e[1;32m修復完成！${NC}"
                read -p "按 Enter 返回子選單..."
                ;;
            3)
                clear
...
verify_and_restart_httpd() {
    ...
    if ! httpd -t >/dev/null 2>&1; then
        echo -e " [\e[1;31mError\e[0m] Apache 設定語法錯誤！請先執行 httpd -t 檢查。"
        return 1
    fi
    ...
    if [ $? -ne 0 ]; then
        echo -e " [\e[1;31mError\e[0m] Apache 服務啟動失敗！請檢查 systemctl status httpd。"
        return 1
    fi

    echo -e " [\e[1;32mOK\e[0m] Apache 服務運行正常。"
    return 0
}
...
# --- http 設定備份函式 ---
quick_backup_httpd() {
    local SOURCE="/etc/httpd"
    local TIMESTAMP=$(date +%Y%m%d_%H%M%S)
    local B_DIR="$SCRIPT_DIR/backups/${TIMESTAMP}_httpd_raw"

    mkdir -p "$B_DIR"
    
    # -a (archive): 等同於 -dR --preserve=all，完整保留權限、連結、時間與 SELinux 標籤
    cp -a "$SOURCE/." "$B_DIR/"
    
    echo -e " [\e[1;32mBackup\e[0m] 已完成原始目錄備份：$B_DIR"
}

# --- http 快速還原函式 ---
quick_restore_httpd() {
    local BACKUP_ROOT="$SCRIPT_DIR/backups"
    local TARGET_DIR="/etc/httpd"

    # 1. 尋找最新的備份目錄 (以 _httpd_raw 結尾的資料夾)
    # ls -td 會按時間排序，head -1 取最上面那個
    local LATEST_BAK=$(ls -td $BACKUP_ROOT/*_httpd_raw 2>/dev/null | head -1)

    # 2. 檢查是否有備份存在
    if [ -z "$LATEST_BAK" ]; then
        echo -e " [\e[1;31mError\e[0m] 找不到任何備份資料夾於 $BACKUP_ROOT"
        return 1
    fi

    echo -e " [\e[1;33mAttention\e[0m] 偵測到最新備份：$LATEST_BAK"
    read -p "確定要將 /etc/httpd 還原至此版本嗎？[y/N]: " confirm

    if [[ "$confirm" =~ ^[Yy]$ ]]; then
        echo "正在執行還原作業..."
        
        # 3. 先將目前的設定搬移至 tmp 避免直接刪除發生意外
        # 這是資深維運的保險做法：還原前再幫「壞掉的現場」留個最後遺言
        mv "$TARGET_DIR" "${TARGET_DIR}_broken_$(date +%H%M%S)"

        # 4. 執行還原 (使用 cp -a 確保權限與 SELinux 標籤)
        mkdir -p "$TARGET_DIR"
        cp -a "$LATEST_BAK/." "$TARGET_DIR/"

        if [ $? -eq 0 ]; then
            echo -e " [\e[1;32mSuccess\e[0m] 還原完成！"
            echo "正在驗證 Apache 語法..."
            httpd -t
            systemctl restart httpd
            echo "Apache 服務已嘗試重新啟動。"
        else
            echo -e " [\e[1;31mError\e[0m] 還原過程中發生錯誤！"
            return 1
        fi
    else
        echo "操作已取消。"
...
for conf in $CONF_FILES; do
    ...
    echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0002 已禁用WebDAV模組"
fi
...
if ! httpd -M 2>/dev/null | grep -q "status_module"; then
    ...
    echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0003 已禁用 Status 模組"
fi

# --- TWGCB-04-007-0004：禁用 mod_autoindex ---
CONF_FILES="/etc/httpd/conf.modules.d/*.conf"
if ! httpd -M 2>/dev/null | grep -q "autoindex_module"; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0004 disabled mod_autoindex"
else
    ...
    echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0004 已自動禁用Autoindex模組"
fi

# --- TWGCB-04-007-0005：禁用 mod_proxy ---
if ! httpd -M 2>/dev/null | grep -q "proxy_module"; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0005 disabled proxy_module"
else
    ...
    echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0005 已自動禁用Proxy模組"
fi

# --- TWGCB-04-007-0006：禁用 mod_userdir ---
CONF_FILES="/etc/httpd/conf.modules.d/*.conf"
if ! httpd -M 2>/dev/null | grep -q "userdir_module"; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0006 disabled userdir_module"
else
    ...
    echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0006 已自動禁用User Directories模組"
fi

# --- TWGCB-04-007-0007：禁用 mod_info ---
if ! httpd -M 2>/dev/null | grep -q "info_module"; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0007 disabled info_module"
else
    CONF_FILES="/etc/httpd/conf.modules.d/*.conf"
    for conf in $CONF_FILES; do
        sed -i '/info_module/s/^/#/' "$conf"
    done
    echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0007 已自動禁用info_module模組"
fi
...
sed -i '/<Directory \/>/,/<\/Directory>/ { /<\/Directory>/i\
    Options None
            ...
            echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0023 $file：<Directory /> 已新增 Options None"
            OK=0
        fi
    fi
done
...
if [ $NEED_FIX -eq 1 ]; then
    ...
    for conf in $CONF_FILES; do
        if [ -f "$conf" ]; then
            ...
            echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0025 已修正 $conf 中存在不安全的Options配置 "
        fi
    done
...
if [ -z "$WELCOME_FILE" ] && [ -z "$MANUAL_INSTALLED" ] && [ ! -f "$WELCOME_CONF" ]; then
    ...
    # 修正步驟 1：只移除 welcome.html (不要刪除 index.html) 
    if [ -n "$WELCOME_FILE" ]; then
        rm -f "$DOC_ROOT"/welcome.html
        echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0026 已移除預設 welcome.html"
    fi

    # 修正步驟 2：處理 welcome.conf (這是最重要的，它控制了測試頁的顯示) 
    if [ -f "$WELCOME_CONF" ]; then
        rm -f "$WELCOME_CONF"
        echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0026 已移除預設測試頁設定檔 (welcome.conf)"
    fi

    # 修正步驟 3：解除安裝手冊套件 
    if [ -n "$MANUAL_INSTALLED" ]; then
        yum remove httpd-manual -y >/dev/null 2>&1
        echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0026 已解除安裝 httpd-manual 套件"
    fi
fi
...
if grep -qE '^\s*TraceEnable\s+off' "$MAIN_CONF"; then
    ...
    echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0030 已更新 TraceEnable 設定為 off $MAIN_CONF"
else
    # 完全沒有設定，於檔案結尾加入
...
# --- TWGCB-04-007-0031：強制 HTTP/1.1 與繼承設定 ---
# TWGCB-04-007-0031 修復 Apache mod_rewrite 與 HTTP/1.0 限制
HTTPD_CONF="/etc/httpd/conf/httpd.conf"
#確認 mod_rewrite 已載入
if ! grep -Eq '^\s*LoadModule\s+rewrite_module' "$HTTPD_CONF"; then
    echo "LoadModule rewrite_module modules/mod_rewrite.so" >> "$HTTPD_CONF"
    echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0031 Added mod_rewrite LoadModule"
else
	echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0031 mod_rewrite 已載入"
fi

#確認主設定有 Rewrite 規則
if ! grep -Eq 'RewriteCond\s+%\{THE_REQUEST\}.*HTTP/1\\\.1' "$HTTPD_CONF"; then
    {
        echo ""
        echo "# TWGCB-04-007-0031 HTTP/1.0 Block"
        echo "RewriteEngine On"
        echo "RewriteCond %{THE_REQUEST} !HTTP/1\.1$"
        echo "RewriteRule .* - [F]"
    } >> "$HTTPD_CONF"
    echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0031 Added HTTP/1.0 blocking rules"
else	
	echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0031 Rewrite 規則存在"
	
fi

#RewriteEngine On 和 RewriteOptions Inherit
CONF_SEARCH_DIRS="/etc/httpd/conf.d"

# 1. 找出所有包含 VirtualHost 的檔案
VHOST_FILES=$(grep -rlEi "<VirtualHost" $CONF_SEARCH_DIRS 2>/dev/null)

for file in $VHOST_FILES; do
    [ -f "$file" ] || continue
    
    # 取得檔案中 VirtualHost 的數量
    BLOCK_COUNT=$(grep -ci "<VirtualHost" "$file")
    ...
    for i in $(seq 1 "$BLOCK_COUNT"); do
        ...
        BLOCK=$(awk "
            /<[Vv]irtual[Hh]ost/{count++; if(count==$i){in_block=1}}
            in_block{print}
            /<\/[Vv]irtual[Hh]ost>/{if(in_block){in_block=0; exit}}
        " "$file")

        # 檢測：如果區塊內缺少 RewriteOptions Inherit
        if ! echo "$BLOCK" | grep -Eqi 'RewriteOptions\s+Inherit'; then
            # 取得該區塊在原檔案中的起始行號 (針對第 i 個匹配項)
            LINE_NUM=$(grep -ni "<VirtualHost" "$file" | sed -n "${i}p" | cut -d: -f1)

            if [ -n "$LINE_NUM" ]; then
                # 在該行號下方插入指令
                sed -i "${LINE_NUM}a \    # TWGCB-04-007-0031\n    RewriteEngine On\n    RewriteOptions Inherit" "$file"
                echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0031 已於 $file 第 $LINE_NUM 行後方補入設定。"
            fi
        else
            echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0031 $file (第 $i 個區塊) 已符合規範。"
        fi
    done
done
...
# --- TWGCB-04-007-0038：日誌格式合規化 (強效跨行清理版) ---
HTTPD_CONF="/etc/httpd/conf/httpd.conf"
CONF_SEARCH_DIRS="/etc/httpd/conf /etc/httpd/conf.d"

# [定義檢測函式] 避免重複寫兩次相同的 grep 邏輯
check_log_format() {
    # 1. 搜尋是否有任何「非 combined」的 CustomLog 啟用中
    local bad_logs=$(grep -rEi "^\s*CustomLog" /etc/httpd/conf* | grep -v "#" | grep -vi "combined")
    # 2. 確認是否至少有一個正確的 combined 設定存在
    local has_good_log=$(grep -rEi "^\s*CustomLog\s+.*combined" /etc/httpd/conf* | grep -v "#")

    if [ -z "$bad_logs" ] && [ -n "$has_good_log" ]; then
        return 0 # 合規
    else
        return 1 # 不合規
    fi
...
if check_log_format; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0038 Access log format is combined."
else
    # 1. 暴力清理：處理所有可能的跨行 CustomLog、LogFormat 與 TransferLog
    # 使用 sed 循環匹配處理帶有反斜線 (\) 的跨行指令
    find $CONF_SEARCH_DIRS -name "*.conf" -exec sed -i '/^\s*CustomLog\b/ { :a; /\\$/ { N; ba }; d }' {} +
    find $CONF_SEARCH_DIRS -name "*.conf" -exec sed -i '/^\s*LogFormat\b/ { :a; /\\$/ { N; ba }; d }' {} +
    find $CONF_SEARCH_DIRS -name "*.conf" -exec sed -i '/^\s*TransferLog\b/ { :a; /\\$/ { N; ba }; d }' {} +

    # 2. 全域寫入 httpd.conf
    # 先清除舊的 TWGCB 標記避免重複追加
    sed -i '/TWGCB-04-007-0038/d' "$HTTPD_CONF"
...
EOF
    ...
    # 3. VirtualHost 處理：在每個虛擬主機區塊內補上 CustomLog
    VHOST_FILES=$(grep -rlEi "<VirtualHost" $CONF_SEARCH_DIRS 2>/dev/null)

    for fpath in $VHOST_FILES; do
        tmpfile=$(mktemp)
        # awk 負責在 </VirtualHost> 結束標籤前插入標準 Log 指令
        awk '
        BEGIN { in_vhost = 0; modified = 0 }
        /^[[:space:]]*<[Vv]irtual[Hh]ost/ { in_vhost = 1; print $0; next }
        in_vhost {
            if ($0 ~ /^[[:space:]]*<\/[Vv]irtual[Hh]ost>/) {
                print "    # TWGCB-04-007-0038 (VHost Log)"
                print "    CustomLog logs/ssl_access_log combined"
                print $0
                in_vhost = 0; modified = 1; next
            }
        }
        { print $0 }
        ' "$fpath" > "$tmpfile"
    ...
    # 4. 驗證與重載
    restorecon -Rv /etc/httpd/ >/dev/null 2>&1
    
    if httpd -t 2>/dev/null; then
        systemctl reload httpd >/dev/null 2>&1
        echo -e " [\e[1;33mFIXED\e[0m] TWGCB-04-007-0038 修復成功，全域與 VHost 日誌已同步。"
    else
        echo -e " [\e[1;31mErr\e[0m] 修復後發現語法錯誤，請手動執行 httpd -t 檢查。"
        ...
        if [ ! -f "$key_file" ]; then
            ...
            if [ $(stat -c %a "$key_file") -ne 400 ] || \
               ...
               [ $(stat -c %G "$key_file") != "root" ]; then
				...
				chmod 0400 "$key_file"
			    echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0041 已修正權限: $key_file"
                ISSUE_FOUND=1
            fi
        fi
    done
...
# 写入安全加密配置
for conf in $CONF_FILES; do
    if grep -qE '^[^#]*<VirtualHost' "$CONF_PATH"; then
		if ! grep -q "^SSLHonorCipherOrder On" "$CONF_PATH" && \
			! grep -q "^SSLCipherSuite ECDHE:DHE:!NULL:!LOW:!SSLv2:!MD5:!RC4:!aNULL" "$CONF_PATH"; then		
        # 虚拟主机配置
		...
		if ! grep -q "^SSLHonorCipherOrder On" "$CONF_PATH" && \
			! grep -q "^SSLCipherSuite ECDHE:DHE:!NULL:!LOW:!SSLv2:!MD5:!RC4:!aNULL" "$CONF_PATH"; then
		# 全局配置
        sed -i '/SSLHonorCipherOrder/d' "$CONF_PATH"
        sed -i '/SSLCipherSuite/d' "$CONF_PATH"
        sed -i -e '$a\' "$CONF_PATH"
		{
		...
		echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0043 $CONF_PATH 已新增配置"
		fi
    fi
done
...
for conf in $CONF_FILES; do
    if grep -q "<VirtualHost" "$conf"; then
		...
		echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0044 SSL重協商配置已修正 $conf"
    else
        # 全局配置
...
# --- TWGCB-04-007-0045：禁用 SSL 壓縮 (精準版) ---
HTTPD_CONF="/etc/httpd/conf/httpd.conf"
SSL_CONF="/etc/httpd/conf.d/ssl.conf"
SEARCH_PATH="/etc/httpd/conf/ /etc/httpd/conf.d/"
...
if [[ "${CURRENT_VAL,,}" == "off" ]]; then
    ...
    # 3. 修復：執行「清道夫」邏輯，確保唯一性
    # 先刪除所有設定檔中的 SSLCompression
    find /etc/httpd/conf/ /etc/httpd/conf.d/ -name "*.conf" -exec sed -i '/^\s*SSLCompression\b/Id' {} +

    # 4. 寫入：為了保險起見，我們寫在 httpd.conf (或是你偏好 ssl.conf)
    # 這裡建議寫在 httpd.conf 末尾，優先權最高
    echo -e "\n# TWGCB-04-007-0045\nSSLCompression off" >> "$HTTPD_CONF"

    # 5. 驗證語法
    if httpd -t 2>/dev/null; then
        systemctl reload httpd
        echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0045 修復完成，已統一設定為 off。"
    else
        echo -e " [\e[1;31mErr\e[0m] 設定語法有誤（可能是 mod_ssl 未安裝），正在還原..."
        sed -i '/# TWGCB-04-007-0045/,+1d' "$HTTPD_CONF"
    fi
fi
 
# --- TWGCB-04-007-0046：OCSP Stapling (精準檢測與修復) ---
SSL_CONF="/etc/httpd/conf.d/ssl.conf"
SEARCH_PATH="/etc/httpd/conf/ /etc/httpd/conf.d/"
...
if [ -n "$HAS_CACHE" ] && [ "$VHOST_MISSING" == "OK" ]; then
    ...
    # A. 處理全域快取 (SSLStaplingCache)
    # 先清理舊的，確保唯一性，寫在 ssl.conf 的全域區段（開頭）
    find /etc/httpd/conf/ /etc/httpd/conf.d/ -name "*.conf" -exec sed -i '/^\s*SSLStaplingCache\b/Id' {} +
    # 插入到 ssl.conf 第一行
    sed -i '1i # TWGCB-04-007-0046 Global Cache\nSSLStaplingCache "shmcb:/run/httpd/stapling_cache(128000)"' "$SSL_CONF"

    # B. 處理 VirtualHost 內部 (SSLUseStapling)
    # 先清理區塊內外的重複項
    sed -i '/^\s*SSLUseStapling\b/Id' "$SSL_CONF"
    # 使用 'a' 指令精準插入到 <VirtualHost> 的下一行
    sed -i '/<VirtualHost[^>]*>/a \    # TWGCB-04-007-0046\n    SSLUseStapling On' "$SSL_CONF"

    # 4. 驗證與重載
    restorecon -v "$SSL_CONF"
    if httpd -t 2>/dev/null; then
        systemctl reload httpd
        echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0046 已完成配置並重載服務。"
    else
        echo -e " [\e[1;31mErr\e[0m] 語法錯誤，請檢查 ssl.conf"
        httpd -t
    fi
fi
...
for conf in $CONF_FILES; do
    ...
    if ! grep -q "<VirtualHost" "$conf"; then
		...
		echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0051 FileETag已配置 $conf"
	fi
    ...
    FileETag None' "$conf"
	echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0051 FileETag已配置 虛擬主機配置 $conf"
    fi
done
fi


# --- TWGCB-04-007-0052：連線逾時 Timeout (最終強效版) ---
HTTPD_CONF="/etc/httpd/conf/httpd.conf"
NEW_TIMEOUT=60
...
# 1. 暴力清除：把所有 .conf 檔案裡的 Timeout 都砍掉 (含 httpd.conf)
# 這樣可以確保沒有重複定義的干擾
find /etc/httpd/conf/ /etc/httpd/conf.d/ -name "*.conf" -exec sed -i '/^\s*Timeout\s\+/d' {} +

# 2. 唯一寫入：只寫在 httpd.conf 的最末尾 (確保最後被讀取)
# 使用 cat 確保 SELinux 標籤 (httpd_config_t) 正確
tmpfile=$(mktemp)
cat "$HTTPD_CONF" > "$tmpfile"
echo -e "\n# TWGCB-04-007-0052\nTimeout $NEW_TIMEOUT" >> "$tmpfile"
cat "$tmpfile" > "$HTTPD_CONF"
rm -f "$tmpfile"


echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0052: 已統一清理並在 $HTTPD_CONF 設置 Timeout=$NEW_TIMEOUT"
fi



# --- TWGCB-04-007-0053：啟用 KeepAlive (最終強效版) ---
HTTPD_CONF="/etc/httpd/conf/httpd.conf"
CONF_SEARCH_PATH="/etc/httpd/conf/ /etc/httpd/conf.d/"

# 1. 取得真正生效的值 (跨檔案檢查最後一筆)
# 使用 [[:space:]] 增加相容性
KEEPALIVE_VAL=$(grep -rEi "^\s*KeepAlive\b" $CONF_SEARCH_PATH 2>/dev/null | grep -v "#" | awk '{print $NF}' | tail -n 1)

# 2. 判斷是否為 "On" (不分大小寫)
if [[ "${KEEPALIVE_VAL,,}" == "on" ]]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0053 KeepAlive 配置正確 (當前值: $KEEPALIVE_VAL)"
else
    # 3. 徹底清理：針對所有 .conf 檔案，只要有 KeepAlive 就砍掉 (避免蓋台)
    # 使用 \b 確保不誤刪 KeepAliveTimeout
    find /etc/httpd/conf/ /etc/httpd/conf.d/ -name "*.conf" -exec sed -i '/^\s*KeepAlive\b/Id' {} +

    # 4. 唯一寫入：在 httpd.conf 末尾寫入正確值
    # 直接使用 >> 追加內容，不會破壞原本檔案的 SELinux 標籤
    echo -e "\n# TWGCB-04-007-0053\nKeepAlive On" >> "$HTTPD_CONF"

    # 5. 強制還原標籤並重載服務
    restorecon -v "$HTTPD_CONF"
    if httpd -t 2>/dev/null; then
        systemctl reload httpd
        echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0053 已清理重複設定，並於 $HTTPD_CONF 啟用 KeepAlive On"
    else
        echo -e " [\e[1;31mErr\e[0m] TWGCB-04-007-0053 修復後語法錯誤，請檢查 $HTTPD_CONF"
        httpd -t
    fi
fi


# --- TWGCB-04-007-0054：限制 MaxKeepAliveRequests ---
HTTPD_CONF="/etc/httpd/conf/httpd.conf"
MAX_REQ=100

# 1. 取得生效值
VAL=$(grep -rEi "^\s*MaxKeepAliveRequests\b" /etc/httpd/conf/ /etc/httpd/conf.d/ 2>/dev/null | grep -v "#" | awk '{print $NF}' | tail -n 1)

if [ "$VAL" == "$MAX_REQ" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0054 MaxKeepAliveRequests 已正確設置為 $MAX_REQ"
else
    # 2. 清理所有檔案中的舊設定
    find /etc/httpd/conf/ /etc/httpd/conf.d/ -name "*.conf" -exec sed -i '/^\s*MaxKeepAliveRequests\b/Id' {} +
    
    # 3. 唯一寫入 httpd.conf
    echo -e "\n# TWGCB-04-007-0054\nMaxKeepAliveRequests $MAX_REQ" >> "$HTTPD_CONF"
    
    restorecon -v "$HTTPD_CONF"
    echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0054 已統一設置 MaxKeepAliveRequests=$MAX_REQ"
fi

# --- TWGCB-04-007-0055：縮短 KeepAliveTimeout ---
HTTPD_CONF="/etc/httpd/conf/httpd.conf"
KA_TIMEOUT=15

# 1. 取得生效值
VAL=$(grep -rEi "^\s*KeepAliveTimeout\b" /etc/httpd/conf/ /etc/httpd/conf.d/ 2>/dev/null | grep -v "#" | awk '{print $NF}' | tail -n 1)

if [ -n "$VAL" ] && [ "$VAL" -le "$KA_TIMEOUT" ] && [ "$VAL" -gt 0 ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0055 KeepAliveTimeout 已合規 (當前值: $VAL)"
else
    # 2. 清理所有檔案中的舊設定 (包含剛才看到的 userdir.conf 殘留)
    find /etc/httpd/conf/ /etc/httpd/conf.d/ -name "*.conf" -exec sed -i '/^\s*KeepAliveTimeout\b/Id' {} +
    
    # 3. 唯一寫入 httpd.conf
    echo -e "\n# TWGCB-04-007-0055\nKeepAliveTimeout $KA_TIMEOUT" >> "$HTTPD_CONF"
    
    restorecon -v "$HTTPD_CONF"
    echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0055 已統一設置 KeepAliveTimeout=$KA_TIMEOUT"
fi
...
# --- TWGCB-04-007-0065：禁用 Basic & Digest 驗證模組 ---
MODULE_CONF="/etc/httpd/conf.modules.d/00-base.conf"

# 1. 檢查模組是否仍處於啟用狀態 (未被註解)
CHECK_BASIC=$(grep -Ei "^\s*LoadModule\s+auth_basic_module" $MODULE_CONF)
CHECK_DIGEST=$(grep -Ei "^\s*LoadModule\s+auth_digest_module" $MODULE_CONF)

if [ -z "$CHECK_BASIC" ] && [ -z "$CHECK_DIGEST" ]; then
    echo -e " [\e[1;32mOK\e[0m] TWGCB-04-007-0065 Basic/Digest 驗證模組已禁用。"
else
    # 2. 執行修復：註解掉這兩個 LoadModule 指令
    # 使用 sed 將開頭不是 # 的 LoadModule 加上 #
    if [ -f "$MODULE_CONF" ]; then
        # 註解 auth_basic_module
        sed -i 's/^\s*LoadModule\s\+auth_basic_module/#&/' "$MODULE_CONF"
        # 註解 auth_digest_module
        sed -i 's/^\s*LoadModule\s\+auth_digest_module/#&/' "$MODULE_CONF"
        
        # 3. 驗證語法
        if httpd -t 2>/dev/null; then
            systemctl restart httpd
            echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0065 已於 $MODULE_CONF 禁用不安全驗證模組。"
        else
            echo -e " [\e[1;31mErr\e[0m] TWGCB-04-007-0065 禁用模組後 Apache 語法錯誤（可能有其他設定依賴這些模組），正在還原..."
            sed -i 's/^#\(LoadModule\s\+auth_basic_module\)/\1/' "$MODULE_CONF"
            sed -i 's/^#\(LoadModule\s\+auth_digest_module\)/\1/' "$MODULE_CONF"
            httpd -t 2>/dev/null
        fi
    else
        echo -e " [\e[1;31mErr\e[0m] TWGCB-04-007-0065 找不到模組設定檔 $MODULE_CONF"
    fi
fi
...
if [ -z "$ERR_MSG" ]; then
   ...
   chown root:root $PID_DIR
   chmod go-w $PID_DIR
   echo -e " [\e[1;33mFIX\e[0m] TWGCB-04-007-0018 PidFile directory $PID_DIR FIX."
fi
...
if [ $? -eq 0 ]; then
    ...
    systemctl restart httpd
	sleep 2
    echo -e " [\e[1;33mINFO\e[0m] 修復完成並已重啟服務。"
else
    echo "警告：Apache 語法檢查失敗，請手動確認設定檔。"
fi
...
# --- TWGCB-01-008-0054：檢查 /etc/passwd- 檔案權限 ---
passwd_bak="/etc/passwd-"
if [ "$(stat -c '%a' $passwd_bak 2>/dev/null)" -le 600 ]; then
    # 注意：這裡要補上 perms 的定義，或者直接在 echo 用指令抓一次
    perms=$(stat -c '%a' $passwd_bak)
    echo -e " [\e[1;32mOK\e[0m] TWGCB-01-008-0054 $passwd_bak permissions are $perms (<=600)"
else
    echo -e " [\e[1;31mKO\e[0m] TWGCB-01-008-0054 $passwd_bak permissions should be <=600 (chmod 600 $passwd_bak)"
fi
...
LOGFILE="$SCRIPT_DIR/TWGCB-01-008-0205.log"
...
LOGFILE="$SCRIPT_DIR/TWGCB-01-008-0241.log"
...
if [ "$(id -u)" -gt 999 ]; then
    # 一般使用者：027 (Owner:rwx, Group:r-x, Others:---)
    umask 027
else
    # 系統帳號 (UID < 1000)：維持最嚴格的 077
    umask 077
fi
EOF
chmod 644 /etc/profile.d/TWGCB.sh
...
# TWGCB-01-008-0271 檢查 SSH 加密演算法設定
...
cat << 'EOF1' >> "$LOGFILE"
修復說明:
將設定寫入/etc/ssh/sshd_config中

# TWGCB-01-008-0271: SSH Encryption, MACs, and Kex Algorithms Hardening
...
CHECK_MAIN_FILE() {
    ...
    if [ ! -f "$file" ]; then
        return # 檔案不存在則跳過，不列入 KO（除非稽核要求檔案必須存在）
    fi

    # 1. 檢查 gpgcheck=1
    if ! grep -iqE '^\s*gpgcheck\s*=\s*1' "$file"; then
        echo "$name: missing or incorrect gpgcheck=1" >> "$LOGFILE"
    fi

    # 2. 檢查 localpkg_gpgcheck=1 (補足這項以免稽核 KO)
    if ! grep -iqE '^\s*localpkg_gpgcheck\s*=\s*1' "$file"; then
        echo "$name: missing or incorrect localpkg_gpgcheck=1" >> "$LOGFILE"
    fi
}
...
CHECK_REPO_FILES() {
    for repo in /etc/yum.repos.d/*.repo; do
        ...
        # 排除 redhat.repo (通常由系統自動生成，手動修改會被覆蓋)
        [[ "$repo" == "/etc/yum.repos.d/redhat.repo" ]] && continue

        # 3. 檢查每個 Repo 的 gpgcheck=1
        if ! grep -iqE '^\s*gpgcheck\s*=\s*1' "$repo"; then
            echo "Repo: $(basename "$repo") gpgcheck is NOT 1" >> "$LOGFILE"
        fi

        # 4. 檢查每個 Repo 的 localpkg_gpgcheck=1
        if ! grep -iqE '^\s*localpkg_gpgcheck\s*=\s*1' "$repo"; then
            echo "Repo: $(basename "$repo") localpkg_gpgcheck is NOT 1" >> "$LOGFILE"
        fi
    done
}
...
if [ -s "$LOGFILE" ]; then
    COUNT=$(wc -l < "$LOGFILE")
    echo -e " [\e[1;31mKO\e[0m] TWGCB-01-012-0032 Found $COUNT GPG issues. (see $LOGFILE)"
else
    echo -e " [\e[1;32mOK\e[0m] TWGCB-01-012-0032 GPG check settings comply with official GCB manual."
fi
...
# --- TWGCB-01-012-0054：檢查 /etc/passwd- 檔案權限 ---
passwd_bak="/etc/passwd-"
if [ "$(stat -c '%a' $passwd_bak 2>/dev/null)" -le 644 ]; then
    # 注意：這裡要補上 perms 的定義，或者直接在 echo 用指令抓一次
    perms=$(stat -c '%a' $passwd_bak)
    echo -e " [\e[1;32mOK\e[0m] TWGCB-01-012-0054 $passwd_bak permissions are $perms (<=644)"
else
    echo -e " [\e[1;31mKO\e[0m] TWGCB-01-012-0054 $passwd_bak permissions should be <=644 (chmod 644 $passwd_bak)"
fi
...
LOGFILE="$SCRIPT_DIR/TWGCB-01-012-0263.log"
...
使用以下指令修復:
# --- 開始複製 ---
cat << 'REPAIR' > "/etc/ssh/sshd_config.d/00-GCB.conf"
...
fix_aide_audit_configs() {
    ...
    # 檢查檔案是否存在
    if [ ! -f "$AIDE_CONF" ]; then
        echo -e " [\e[1;31mERR\e[0m] TWGCB-01-012-0145 $AIDE_CONF 不存在，跳過此項。"
        return 1
    fi
...
fix_auditd_conf_limits() {
    ...
    if [ ! -f "$CONF" ]; then
        echo -e " [\e[1;31mERR\e[0m] TWGCB-01-008-0146 $CONF 不存在，跳過此項。"
        return 1
    fi
...
EOF
    ...
    if visudo -cf "$SUDO_CONF" >/dev/null 2>&1; then
        ...
        echo -e " [\e[1;31mERR\e[0m] TWGCB-01-012-0034/0035 sudo configuration syntax error, removing invalid config."
        rm -f "$SUDO_CONF"
    fi
else
